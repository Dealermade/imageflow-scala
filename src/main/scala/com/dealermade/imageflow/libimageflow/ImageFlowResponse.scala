...
case class VersionInfoResponse(
  code: Int,
  success: Boolean,
  message: String,
...
case class BuildResultResponse(
  code: Int,
  success: Boolean,
  message: String,
  data: Option[JobResult]
) extends ImageFlowResponse
case class BuildResult(@jsonField("build_result") buildResult: JobResultPayload)

case class JobResultResponse(
  code: Int,
  success: Boolean,
  message: String,
  data: Option[JobResult]
...
object ImageFlowResponse {
  implicit val encodeResultDecoder: JsonDecoder[EncodeResult] =
    DeriveJsonDecoder.gen[EncodeResult]
  implicit val decodeResultDecoder: JsonDecoder[DecodeResult] =
    DeriveJsonDecoder.gen[DecodeResult]
  implicit val nodePerfDecoder: JsonDecoder[NodePerf] = DeriveJsonDecoder.gen[NodePerf]
  implicit val framePerformanceDecoder: JsonDecoder[FramePerformance] =
    DeriveJsonDecoder.gen[FramePerformance]
  implicit val buildPerformanceDecoder: JsonDecoder[BuildPerformance] =
    DeriveJsonDecoder.gen[BuildPerformance]
  implicit val versionInfoPayloadDecoder: JsonDecoder[VersionInfoPayload] =
    DeriveJsonDecoder.gen[VersionInfoPayload]
  implicit val imageInfoPayloadDecoder: JsonDecoder[ImageInfoPayload] =
    DeriveJsonDecoder.gen[ImageInfoPayload]
  implicit val jobResultPayloadDecoder: JsonDecoder[JobResultPayload] =
    DeriveJsonDecoder.gen[JobResultPayload]
  implicit val imageInfoDecoder: JsonDecoder[ImageInfo] = DeriveJsonDecoder.gen[ImageInfo]
  implicit val versionInfoDecoder: JsonDecoder[VersionInfo] =
    DeriveJsonDecoder.gen[VersionInfo]
  implicit val jobResultDecoder: JsonDecoder[JobResult] = DeriveJsonDecoder.gen[JobResult]
  implicit val imageInfoResponseDecoder: JsonDecoder[ImageInfoResponse] =
    DeriveJsonDecoder.gen[ImageInfoResponse]
  implicit val versionInfoResponseDecoder: JsonDecoder[VersionInfoResponse] =
    DeriveJsonDecoder.gen[VersionInfoResponse]
  implicit val buildResultResponseDecoder: JsonDecoder[BuildResultResponse] =
    DeriveJsonDecoder.gen[BuildResultResponse]
  implicit val jobResultResponseDecoder: JsonDecoder[JobResultResponse] =
    DeriveJsonDecoder.gen[JobResultResponse]
  implicit val decoder: JsonDecoder[ImageFlowResponse] =
    DeriveJsonDecoder.gen[ImageFlowResponse]

  def parse(jsonResponse: String): Task[JobResultResponse] = {
    import zio.json._
    import com.dealermade.imageflow.libimageflow.ImageFlowResponse._

    val last = jsonResponse.toCharArray.lastIndexOf('}')
    val before = jsonResponse.slice(0, last + 1)
    Task.fromEither(
      before
        .fromJson[JobResultResponse]
        .left
        .map(errorString => new Throwable(errorString))
    )
  }
